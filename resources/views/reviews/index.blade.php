...
@section('title', __('messages.product_reviews_page_title'))

@section('breadcrumb')
<a href="{{ route($dashboardRoute::name('index')) }}">{{ __('messages.dashboard') }}</a>
<span class="breadcrumb-separator">/</span>
<span>{{ __('messages.product_reviews_page_title') }}</span>
@endsection
...
<div class="page-header">
    <div class="page-header-row">
        <div>
            <h1 class="page-title">{{ __('messages.product_reviews_page_title') }}</h1>
            <p class="page-description">{{ __('messages.product_reviews_description') }}</p>
        </div>
    </div>
</div>
        ...
        <form action="{{ route('reviews.index') }}" method="GET">
            <div class="filters-bar">
                <div class="search-box">
                    ...
                    <input type="text" name="search" class="form-input" placeholder="{{ __('messages.search_reviews_placeholder') }}" value="{{ $filters['search'] ?? '' }}">
                </div>
                <div class="filter-group">
                    <label class="filter-label">{{ __('messages.rating') }}</label>
                ...
                @if(array_filter($filters, fn($v) => $v !== null && $v !== ''))
                    <a href="{{ route('reviews.index') }}" class="btn btn-ghost">{{ __('messages.clear') }}</a>
                @endif
            </div>
        </form>
    </div>
...
<div class="card">
    ...
    <div class="table-container">
        <table class="table">
            <thead>
                <tr>
                    <th style="width:200px;">{{ __('messages.customer') }}</th>
                    <th style="width:240px;">{{ __('messages.product_name') }}</th>
                    <th style="width:130px;">{{ __('messages.rating') }}</th>
                    <th>{{ __('messages.comment') }}</th>
                    <th style="width:140px;">{{ __('messages.submitted_at') }}</th>
                    <th style="text-align:right; width:80px;">{{ __('messages.actions') }}</th>
                </tr>
            </thead>
            <tbody>
                ...
                <tr>
                    <td>
                        <div style="font-weight:600; font-size:0.9375rem; color:var(--foreground);">{{ $review->user ? $review->user->name : __('messages.unknown_user') }}</div>
                    ...
                    <td>
                        ...
                        @else
                            <span style="color:var(--muted-foreground); font-size:0.875rem;">{{ __('messages.deleted_product') }}</span>
                        @endif
                    </td>
                    <td>
                        ...
                        <div class="action-buttons" style="justify-content:flex-end;">
                            <form action="{{ route('reviews.destroy', $review->id) }}" method="POST" style="display:inline;" id="delete-review-{{ $review->id }}">
                                ...
                                <button type="button" class="action-btn action-btn-danger" title="{{ __('messages.delete') }}"
                                    onclick="showDanger('{{ __('messages.delete_review') }}', '{{ __('messages.delete_review_confirm') }}').then(confirmed => { if(confirmed) document.getElementById('delete-review-{{ $review->id }}').submit(); })">
                                    <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
    ...
    <div style="padding: 1rem 1.5rem; border-top: 1px solid var(--border); display:flex; align-items:center; justify-content:space-between; flex-wrap:wrap; gap:0.75rem;">
        <div style="font-size:0.875rem; color:var(--muted-foreground);">
            {{ __('messages.showing') }} {{ $reviews->firstItem() }}–{{ $reviews->lastItem() }} {{ __('messages.of') }} {{ $reviews->total() }} reviews
        </div>
    ...
    <div class="empty-state">
        ...
        <h3 class="empty-state-title">{{ __('messages.no_reviews_found') }}</h3>
        <p class="empty-state-description">
            @if(array_filter($filters, fn($v) => $v !== null && $v !== ''))
                {{ __('messages.no_reviews_match') }}
            @else
                {{ __('messages.reviews_will_show') }}
            @endif
        </p>
        @if(array_filter($filters, fn($v) => $v !== null && $v !== ''))
            <div style="display:flex; justify-content:center; margin-top:0.75rem;">
                <a href="{{ route('reviews.index') }}" class="btn btn-secondary">{{ __('messages.clear_filters') }}</a>
            </div>
        @endif
    </div>
    @endif
