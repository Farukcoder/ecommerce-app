...
<div class="page-header">
    <div class="page-header-row">
        <div>
            <h1 class="page-title">{{ __('messages.orders_page_title') }}</h1>
            <p class="page-description">{{ __('messages.orders_description') }}</p>
        </div>
    </div>
</div>
...
<div class="stats-grid" style="grid-template-columns: repeat(auto-fit, minmax(160px, 1fr)); margin-bottom: 1.5rem;">
    <div class="stat-card">
        <div class="stat-content">
            <div class="stat-label">{{ __('messages.total_orders_today') }}</div>
            <div class="stat-value" style="font-size:1.5rem;">{{ $summary['total_orders'] }}</div>
        </div>
    </div>
    <div class="stat-card">
        <div class="stat-content">
            <div class="stat-label">{{ __('messages.total_revenue_today') }}</div>
            <div class="stat-value" style="font-size:1.5rem;">@money($summary['total_revenue'])</div>
        </div>
    </div>
    <div class="stat-card">
        <div class="stat-content">
            <div class="stat-label">{{ __('messages.pending_today') }}</div>
            <div class="stat-value" style="font-size:1.5rem;">{{ $summary['pending_count'] }}</div>
        </div>
    </div>
    <div class="stat-card">
        <div class="stat-content">
            <div class="stat-label">{{ __('messages.cancelled_today') }}</div>
        ...
        <form action="{{ route('orders.index') }}" method="GET">
            <div class="filters-bar">
                <div class="search-box">
                    ...
                    <input type="text" name="search" class="form-input" placeholder="{{ __('messages.search_order_placeholder') }}" value="{{ $filters['search'] ?? '' }}">
                </div>
                <div class="filter-group">
                    <label class="filter-label">{{ __('messages.status_filter') }}</label>
                ...
                <div class="filter-group">
                    <label class="filter-label">{{ __('messages.payment_filter') }}</label>
                ...
                <div class="filter-group">
                    <label class="filter-label">{{ __('messages.method_filter') }}</label>
                ...
                <div class="filter-group">
                    <label class="filter-label">{{ __('messages.from_label') }}</label>
                    <input type="text" name="date_from" class="form-input" value="{{ $filters['date_from'] ?? '' }}" data-calendar-field data-placeholder="{{ __('messages.pick_start_date') }}" autocomplete="off">
                </div>
                <div class="filter-group">
            ...
            <div style="display:flex; align-items:center; gap:0.75rem; width:100%;">
                ...
                <div style="margin-left:auto; display:flex; gap:0.5rem; align-items:center;">
                    <select name="status" class="form-select" style="min-width:160px;">
                        <option value="">{{ __('messages.bulk_status_update') }}</option>
                        @foreach(['processing','shipped','delivered','cancelled'] as $status)
                            <option value="{{ $status }}">{{ ucfirst(__('messages.order_status_' . $status)) }}</option>
                        @endforeach
                    </select>
                    <button type="submit" class="btn btn-sm btn-secondary">{{ __('messages.apply_button') }}</button>
                </div>
            </div>
        </div>
            ...
            <table class="table">
                <thead>
                    <tr>
                        ...
                        <th>{{ __('messages.order_number_header') }}</th>
                        <th>{{ __('messages.customer_header') }}</th>
                        <th>{{ __('messages.date_header') }}</th>
                        <th>{{ __('messages.items_header') }}</th>
                        <th>{{ __('messages.total_header') }}</th>
                        <th>{{ __('messages.payment_header') }}</th>
                        <th>{{ __('messages.status_header') }}</th>
                        <th style="text-align:right;">{{ __('messages.actions_header') }}</th>
                    </tr>
                </thead>
                <tbody>
                    @forelse($orders as $order)
                        <tr>
                            ...
                            <td>
                                ...
                                @else
                                    <div style="font-weight:500; color:var(--muted-foreground);">{{ __('messages.guest_label') }}</div>
                                @endif
                            ...
                            <td>
                                <div>{{ $order->payment_method ? __('messages.payment_method_' . $order->payment_method) : __('messages.n_a') }}</div>
                                <div style="font-size:0.8125rem; color:var(--muted-foreground);">{{ __('messages.payment_status_' . $order->payment_status) }}</div>
                            </td>
                            <td>
                                @php
                                    $statusColors = [
                            ...
                            <td style="text-align:right;">
                                <a href="{{ route('orders.show', $order) }}" class="btn btn-sm btn-secondary">{{ __('messages.view_button') }}</a>
                                <a href="{{ route('orders.invoice', $order) }}" target="_blank" class="btn btn-sm btn-ghost">{{ __('messages.invoice_button') }}</a>
                            </td>
                        </tr>
                    @empty
                        <tr>
...
<script>
    ...
    const updateSelectedCount = () => {
        ...
        const selectedText = '{{ __('messages.selected_count') }}'.replace(':count', count);
