...
<div class="page-header">
    <div class="page-header-row">
        <div>
            <h1 class="page-title">{{ __('messages.edit_product') }}</h1>
            <p class="page-description">{{ __('messages.edit_product_description') }}</p>
        </div>
    ...
    <div>
        ...
        <div class="card" style="margin-bottom:1.25rem;">
            ...
            <div class="card-body">
                <div class="form-row" style="grid-template-columns:1fr 1fr; margin-bottom:1.25rem;">
                    <div class="form-group" style="margin-bottom:0;">
                        ...
                        <div id="thumbnail-preview" style="{{ $product->thumbnail ? '' : 'display:none;' }} margin-top:0.75rem; position:relative; width:100%; aspect-ratio:1; border-radius:8px; overflow:hidden; border:1px solid var(--border);">
                            <img id="thumbnail-img" src="{{ $product->thumbnail ? asset('storage/'.$product->thumbnail) : '' }}" alt="{{ __('messages.thumbnail') }}" style="width:100%;height:100%;object-fit:cover;">
                ...
                <p style="font-size:0.8125rem; color:var(--muted-foreground); margin-top:0.5rem;" id="gallery-hint" style="{{ $product->images->count() ? '' : 'display:none;' }}">{{ __('messages.click_set_primary') }}</p>
            </div>
        </div>
            ...
            <div class="card-body">
                <div class="form-group">
                    ...
                    <select name="_sidebar_status" id="sidebar-status" class="form-select" onchange="document.getElementById('form-status').value=this.value">
                        <option value="draft" {{ strtolower(old('status', $product->status)) === 'draft' ? 'selected' : '' }}>{{ __('messages.draft') }}</option>
                        <option value="published" {{ strtolower(old('status', $product->status)) === 'published' ? 'selected' : '' }}>{{ __('messages.published') }}</option>
                        <option value="archived" {{ strtolower(old('status', $product->status)) === 'archived' ? 'selected' : '' }}>{{ __('messages.archived') }}</option>
                    </select>
                </div>
                ...
                <div class="price-row" style="margin-top:0;">
                    <div class="form-group" style="margin-bottom:0;">
                        ...
                        <select id="discount_type" name="discount_type" class="form-select">
                            ...
                            <option value="percentage" {{ old('discount_type', $product->discount_type)==='percentage' ? 'selected' : '' }}>{{ __('messages.percentage') }}</option>
                            <option value="fixed" {{ old('discount_type', $product->discount_type)==='fixed' ? 'selected' : '' }}>{{ __('messages.fixed') }}</option>
                        </select>
...
function renderVariantRows(rows) {
    ...
    tbody.innerHTML = '';
    rows.forEach((row, i) => {
        const label = row.label || `{{ __('messages.variant') }} ${i + 1}`;
        ...
        tr.innerHTML = `
            ...
            <td><select name="variants[${i}][status]" class="form-select" style="font-size:0.8125rem;padding:0.375rem 1.5rem 0.375rem 0.625rem;">
                <option value="in_stock" ${row.status === 'in_stock' ? 'selected' : ''}>{{ __('messages.in_stock') }}</option>
                <option value="out_of_stock" ${row.status === 'out_of_stock' ? 'selected' : ''}>{{ __('messages.out_of_stock') }}</option>
                <option value="backorder" ${row.status === 'backorder' ? 'selected' : ''}>{{ __('messages.backorder') }}</option>
            </select></td>
