...
@section('title', __('messages.categories_page_title'))

@section('breadcrumb')
<a href="{{ route($dashboardRoute::name('index')) }}">{{ __('messages.dashboard') }}</a>
<span class="breadcrumb-separator">/</span>
<span>{{ __('messages.categories_page_title') }}</span>
@endsection
...
<div class="page-header">
    <div class="page-header-row">
        <div>
            <h1 class="page-title">{{ __('messages.categories_page_title') }}</h1>
            <p class="page-description">{{ __('messages.categories_description') }}</p>
        </div>
        ...
        <form action="{{ route('categories.index') }}" method="GET">
            <div class="filters-bar">
                ...
                @if(array_filter($filters, fn($v) => $v !== null && $v !== ''))
                    <a href="{{ route('categories.index') }}" class="btn btn-ghost">{{ __('messages.clear') }}</a>
                @endif
            </div>
        </form>
    </div>
...
<div class="card">
                ...
                <tr>
                    ...
                    <td>
                        <div class="action-buttons" style="justify-content:flex-end;">
                            <a href="{{ route('categories.edit', $category) }}" class="action-btn" title="{{ __('messages.edit') }}">
                                <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
                            ...
                            <form action="{{ route('categories.destroy', $category->id) }}" method="POST" style="display:inline;" id="delete-category-{{ $category->id }}">
                                ...
                                <button type="button" class="action-btn action-btn-danger" title="{{ __('messages.delete') }}"
                                    onclick="showDanger('{{ __('messages.delete') }}', '{{ __('messages.delete_category_confirm', ['name' => addslashes($category->name)]) }}').then(confirmed => { if(confirmed) document.getElementById('delete-category-{{ $category->id }}').submit(); })">
                                    <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
    ...
    <div style="padding: 1rem 1.5rem; border-top: 1px solid var(--border); display:flex; align-items:center; justify-content:space-between; flex-wrap:wrap; gap:0.75rem;">
        <div style="font-size:0.875rem; color:var(--muted-foreground);">
            {{ __('messages.showing') }} {{ $categories->firstItem() }}–{{ $categories->lastItem() }} {{ __('messages.of') }} {{ $categories->total() }} {{ __('messages.categories') }}
        </div>
    ...
    <div class="empty-state">
        ...
        <h3 class="empty-state-title">{{ __('messages.no_categories_found') }}</h3>
        <p class="empty-state-description">
            @if(array_filter($filters, fn($v) => $v !== null && $v !== ''))
                {{ __('messages.no_categories_match') }}
            @else
        ...
        <div style="display:flex; gap:0.75rem; justify-content:center; flex-wrap:wrap;">
            ...
            @if(array_filter($filters, fn($v) => $v !== null && $v !== ''))
                <a href="{{ route('categories.index') }}" class="btn btn-secondary">{{ __('messages.clear_filters') }}</a>
            @endif
        </div>
    </div>
    @endif
